...
USD_rates['days length_coef'] = ((USD_rates['EndDate']-USD_rates['StartDate']) / pd.to_timedelta(1, unit='D'))/360
USD_rates['Implied_rate_12m'] = USD_rates['Implied_rate_3m']*USD_rates['days length_coef']
USD_rates['Implied_rate_12m'].astype(str)
USD_rates['Conv, adj'].astype(str)
USD_rates['Forward rate'] = USD_rates['Implied_rate_3m']-USD_rates['Conv, adj']
USD_rates['DF']=1/(1+USD_rates['Implied_rate_12m']*1)
USD_rates['DF1']=USD_rates['Implied_rate_3m']-(USD_rates['Forward rate']*(1+USD_rates['Implied_rate_3m']*USD_rates['days length_coef']))/(USD_rates['days length_coef']*1)
USD_rates['Implied_rate_3m'][:1] = USD_rates['Market price'][:1]
plt.plot(USD_rates['EndDate2'][1:13],USD_rates['DF1'][1:13],'-', USD_rates['EndDate2'][1:13], USD_rates['Forward rate'][1:13], '--' )
